...
## ð First-Time Setup (Do This Once)
...
First run also bootstraps the environment automatically (creates `.venv`, installs dependencies, installs Chrome for Patchright) â this can take a minute or two the very first time. If it says `Authenticated: No`, continue to step 2.

### 2. Log in â but only where a visible browser is actually reachable
...
- **Claude Code running locally on your own machine**: this works as expected â a browser window opens on your screen, you log in, done.
- **Cowork / any cloud/headless session**: there is no screen you can see, so a browser opened here is invisible to you and the login will always time out. Do not run `setup` here. Instead:
  ...
  - `state.json` holds live Google session cookies â treat it like a password. Don't paste its contents into chat or leave copies lying around after transplanting it; delete the copy in transit once it's in place.
...
See "Add Command - Smart Discovery" below â don't skip straight to asking questions on an empty library.

## ð MANDATORY FIRST STEP â Always check the local `.md` cache before querying

This applies to **every** notebook, unconditionally â whether or not its `library.json` description happens to mention a cache file. Do not skip this because a description looks generic or old.
...
2. Check whether `data/notes/<notebook-id>.md` exists â `Read` it if it does, or `ls data/notes/` first if unsure of the exact filename.
...
5. After getting new answers, update the cache file (create it under `data/notes/<notebook-id>.md` if it doesn't exist yet â sanitize `/` in the ID first) so the next session doesn't have to re-ask the same thing.

Why this is non-negotiable: NotebookLM has no session persistence and a daily quota that runs out fast (see "Known Quirks" below). The `.md` cache is the *only* thing that survives across sessions and environments â nothing else in this skill remembers past answers. Skipping the cache check wastes quota re-fetching things that are already known, and skipping the cache update throws away work the next session will have to redo from scratch.
...
## â ï¸ CRITICAL: Add Command - Smart Discovery
...
## â ï¸ CRITICAL: Local Knowledge Cache â Check FIRST, Update ALWAYS
...
- Notebook IDs can contain filesystem-unsafe characters (e.g. an ID derived from a name like "...2025/2026" contains a literal `/`) â sanitize `/` â `-`) before using an ID as a filename, or you'll create unwanted subdirectories. `notebook_manager.py add` now does this automatically for new notebooks; check existing IDs in `library.json` before assuming they're already safe.
...
**Workflow:**
1. **BEFORE** querying â check whether the cache already answers the question. This saves quota.
2. **AFTER** a substantial Q&A block â update the cache, organized by topic, not chronologically.

This applies whether you're running in Claude Code or in Cowork â the cache is what makes cross-session (and cross-environment) continuity possible at all, since neither the NotebookLM chat itself nor this skill's own scripts persist Q&A history anywhere.
...
- `notes/<notebook-id>.md` - Canonical local knowledge cache per notebook (see "Local Knowledge Cache" above). Not created automatically â build it up as you go.

**Security:** Protected by `.gitignore`, never commit to git. `browser_state/state.json` in particular is equivalent to a password (it holds live Google session cookies) â treat it with the same care, whether it's generated locally or transplanted from another environment.
...
Activate notebook â python scripts/run.py notebook_manager.py activate --id ID
    â
Check local cache first â data/notes/<notebook-id>.md (saves quota, may already have the answer)
    â
...
10. **For scanned PDFs without an OCR text layer** - NotebookLM/Gemini can read them multimodally, but ask about content ("What is X?"), not structure ("What's on page Y of file Z?") â structural references fail more often than content questions on the same source

## Limitations

- No session persistence (each question = new browser); the skill itself keeps no Q&A history â the local cache convention above is the only thing that survives across sessions
- Rate limits on free Google accounts (50 queries/day), and exhaustion is silent at the content level (see Troubleshooting) â behavior may vary by Google account/region
