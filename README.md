...
# Gemini Notebook — NotebookLM Skill for Claude Code / Cowork
...
A Claude skill that queries your Google NotebookLM notebooks through browser automation and returns Gemini's source-grounded answers directly into your conversation — no manual copy-paste between NotebookLM and your editor.
...
See the **"🚀 First-Time Setup"** section at the top of `SKILL.md` for the full walkthrough — checking auth status, logging in (visible-browser requirement and how to handle that in a headless/cloud session), and validating the login actually works before adding notebooks.
...
- Fixed a domain-redirect bug (`notebooklm.google.com` → `notebook.google.com`)
...
- Notebook IDs are sanitized (`/` → `-`) so names containing a slash don't create unwanted subfolders
