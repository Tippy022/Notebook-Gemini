...
QUOTA_EXHAUSTED_MARKERS = [
    "dagelijkse chatlimiet",       # NL: "U heeft uw dagelijkse chatlimiet bereikt"
    "daily chat limit",            # EN equivalent
    "kom later terug",             # NL follow-on text
    "come back later",             # EN follow-on text
]


def _check_quota_exhausted(page) -> bool:
    """Best-effort check of the full page text for the quota-limit banner."""
    try:
        body_text = page.inner_text("body").lower()
        return any(marker in body_text for marker in QUOTA_EXHAUSTED_MARKERS)
    except Exception:
        return False
...
def ask_notebooklm(question: str, notebook_url: str, headless: bool = True) -> str:
    ...
    try:
        ...
        # Wait for NotebookLM
        # NOTE: Google appears to have moved/renamed NotebookLM's canonical domain from
        # notebooklm.google.com to notebook.google.com (observed redirects to the shorter
        # domain). Match both so this doesn't hard-timeout on the wait_for_url below.
        page.wait_for_url(re.compile(r"^https://notebook(lm)?\.google\.com/"), timeout=10000)

        # Dismiss any leftover overlay/backdrop from the "clear chat history" step above.
        # Angular CDK overlays sometimes fail to close cleanly and then block every click
        # on the query input with "element intercepts pointer events" before we've even
        # typed anything. Escape + a forced click on the backdrop clears it reliably.
        try:
        ...
        if not answer:
            if _check_quota_exhausted(page):
                ...
                return (
                    ...
                    "every question will fail the same way until the quota resets (or you "
                    "switch accounts). Do not retry with a simpler question; tell the user "
...
def main():
    ...
    # Loud, unmissable reminder: check the local cache BEFORE spending quota on NotebookLM.
    # This fires regardless of what the notebook's library.json description says, so it
    # can't be skipped just because a description looks generic or predates the cache
    # convention.
