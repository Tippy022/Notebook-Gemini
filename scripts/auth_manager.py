...
class AuthManager:
    ...
    def is_authenticated(self) -> bool:
        ...
        if age_days > 7:
            print(f"â ï¸ Browser state is {age_days:.1f} days old, may need re-authentication")
    ...
    def setup_auth(self, headless: bool = False, timeout_minutes: int = 10) -> bool:
        ...
        """
        print("ð Starting authentication setup...")
        ...
        try:
            ...
            # (Google has been observed redirecting notebooklm.google.com to the shorter
            # notebook.google.com -- accept either.)
            if (("notebooklm.google.com" in page.url or "notebook.google.com" in page.url)
                    and "accounts.google.com" not in page.url):
                print("  â Already authenticated!")
                self._save_browser_state(context)
                return True

            # Wait for manual login
            print("\n  â³ Please log in to your Google account...")
            print(f"  â±ï¸  Waiting up to {timeout_minutes} minutes for login...")

            try:
                # Wait for URL to change to NotebookLM (regex ensures it's the actual domain, not a parameter)
                # Accept both notebooklm.google.com and the shorter notebook.google.com --
                # Google has been observed redirecting to the latter.
                ...
                print(f"  â Authentication timeout: {e}")
                return False
    ...
    def _save_browser_state(self, context: BrowserContext):
        ...
        try:
            ...
            print(f"  ð¾ Saved browser state to: {self.state_file}")
        except Exception as e:
            print(f"  â Failed to save browser state: {e}")
            raise
    ...
    def re_auth(self, headless: bool = False, timeout_minutes: int = 10) -> bool:
        ...
        Returns:
            True if successful
        """
        print("ð Starting re-authentication...")
    ...
    def validate_auth(self) -> bool:
        ...
        print("ð Validating authentication...")
        ...
        try:
            ...
            if (("notebooklm.google.com" in page.url or "notebook.google.com" in page.url)
                    and "accounts.google.com" not in page.url):
                print("  â Authentication is valid")
                return True
            else:
                print("  â Authentication is invalid (redirected to login)")
                return False
...
def main():
    ...
    if args.command == 'setup':
        if auth.setup_auth(headless=args.headless, timeout_minutes=args.timeout):
            print("\nâ Authentication setup complete!")
            print("You can now use ask_question.py to query NotebookLM")
        else:
            print("\nâ Authentication setup failed")
            exit(1)

    elif args.command == 'status':
        info = auth.get_auth_info()
        print("\nð Authentication Status:")
        ...
        if auth.re_auth(timeout_minutes=args.timeout):
            print("\nâ Re-authentication complete!")
        else:
            print("\nâ Re-authentication failed")
            exit(1)
