...
class NotebookLibrary:
    ...
    def add_notebook(
        ...
        """
        # Generate ID from name. Sanitize filesystem-unsafe characters -- names like
        # "Audit & Assurance (Windesheim 2025/2026)" contain a literal "/", which would
        # otherwise produce a notebook ID that creates unwanted subdirectories wherever
        # it's used as a filename (e.g. the data/notes/<id>.md cache convention).
        notebook_id = name.lower().replace(' ', '-').replace('_', '-').replace('/', '-')
...
def main():
    ...
    # Update command
    update_parser = subparsers.add_parser('update', help='Update an existing notebook\'s metadata')
    ...
    update_parser.add_argument('--topics', help='New comma-separated topics (replaces existing)')
    update_parser.add_argument('--use-cases', help='New comma-separated use cases (replaces existing)')
    update_parser.add_argument('--tags', help='New comma-separated tags (replaces existing)')
